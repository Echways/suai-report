...
- Запятые в значениях можно писать без скобок:
  `teacher-post = доцент, канд. техн. наук`. Длинный текст над линией
...
| `\suaicode[язык]{файл}{Подпись}[метка]` | листинг из файла (ищется и в `code/`), метка `lst:файл` |
...
`eq`, `lst`) и нажми **Tab**. Первая табуляция сниппета — метка, она же
подставляется в `\figref`/`\tabref`/`\lstref`/`\formref` рядом. Внутри
`\figref{…}` имена рисунков подсказываются сами.
...
| Интервал как «1,5» в Word (24 pt) | `\setstretch{1.42}` |
...
| Ключевые слова полужирным | `\lstset{keywordstyle=\bfseries}` |
| Короткое тире в подписях | `\captionsetup{labelsep=endash}` |
